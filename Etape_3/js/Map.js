...
class Map {
   ...
   createPlayers() {
      ...
      this.game.players.forEach(el => {
         ...
         tdEltId
            ...
            .addClass(`${tdEltId.attr("data-player")} player`);
      });
   }
   ...
   displayMoves(player) {
      ...
      // pushing each position of the cells regrouped by line into the posArray object
      ...
      for (let i = 0; i < posArrayKeys.length; i++) { //each line
         ...
         posArray[actualKey].forEach(el => { // in each tab
            ...
            // if it's an obstacle
               ...
               posArray[actualKey].splice(
                  obstacleIndex,
                  posArrayKeys.length - 1
               );
            }
         });
      }
